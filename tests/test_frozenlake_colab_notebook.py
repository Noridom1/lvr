...
class FrozenLakeColabNotebookTest(unittest.TestCase):
    def test_notebook_code_compiles_and_uses_task_requirements(self) -> None:
        ...
        self.assertIn("FIXED_SAMPLE_SPLIT", combined_source)
        self.assertIn("FIXED_LVR_STEPS = 259", combined_source)
        self.assertIn('"--decoding-strategy",\n            "fixed"', combined_source)
        self.assertIn('"--fixed-lvr-steps"', combined_source)
    ...
    def test_evaluator_separates_latent_placeholders_from_action_output(self) -> None:
        ...
        self.assertIn('"fixed_steps"', eval_source)
        self.assertIn("DEFAULT_FIXED_LVR_STEPS = 259", eval_source)
        self.assertIn('decoding_strategy="fixed" if fixed_decoding else "latent"', eval_source)
        self.assertIn('decoding_strategy in {"steps", "fixed"}', model_source)
        self.assertIn("boundary_token_id = self.config.lvr_latent_end_id", model_source)
